...
#define WIN32_INPUT_DEVICES_STATIC_INIT                                        \
    {                                                                          \
...
/// @summary Define global data associated with the low-level input system.
struct WIN32_INPUT_SYSTEM
{
    WIN32_INPUT_DEVICES  Buffers[2];            /// Storage for previous and current input device state.
    WIN32_INPUT_DEVICES *PrevState;             /// Pointer to the buffer containing state data from the previous tick.
    WIN32_INPUT_DEVICES *CurrState;             /// Pointer to the buffer containing state data for the current tick.
    uint32_t             PrevPorts;             /// Bitvector for gamepad ports connected on the previous tick.
    uint32_t             CurrPorts;             /// Bitvector for gamepad ports connected on the current tick.
